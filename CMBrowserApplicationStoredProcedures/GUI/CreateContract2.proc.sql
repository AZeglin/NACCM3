IF EXISTS ( SELECT * FROM sysobjects WHERE type = 'P' AND name = 'CreateContract2' )
BEGIN
	DROP PROCEDURE CreateContract2
END
GO

CREATE PROCEDURE CreateContract2
(
...
@SolicitationNumber nvarchar(40) = null,  -- added 3/5/21
...
BEGIN TRANSACTION
		...
		/* this version of SP for release 2 allows for selection of parent contract upon BPA creation */
		...
		BEGIN
			select @OfferId = null
		END				
		/* note: default Returned_Goods_Policy_Type to 3 which is "none selected" vs. null */
		insert Into tbl_Cntrcts
			( CntrctNum, Schedule_Number, CO_ID, Contractor_Name, Solicitation_Number,
 			Primary_Address_1, Primary_Address_2, Primary_City, Primary_State, Primary_Zip, Primary_CountryId,
 			POC_Primary_Name, POC_Primary_Phone, POC_Primary_Ext, POC_Primary_Fax, POC_Primary_Email, POC_VendorWeb, Returned_Goods_Policy_Type,				
			Dates_CntrctAward, Dates_Effective, Dates_CntrctExp, BPA_FSS_Counterpart, Offer_ID, RebateRequired, Socio_Business_Size_ID, CreatedBy, CreationDate, LastModifiedBy, LastModificationDate ) -- Dates_TotOptYrs, Drug_Covered,
		values
			( @ManuallyAssignedContractNumber, @ScheduleNumber, @AssignedCOID, @VendorName, @SolicitationNumber,
			@VendorAddress1, @VendorAddress2, @VendorAddressCity, @VendorAddressState, @VendorZipCode, @VendorCountryId,
			@VendorContactName, @VendorContactPhone, @VendorContactPhoneExtension, @VendorContactFax, @VendorContactEmail, @VendorUrl, 3,
			@AwardDate, @EffectiveDate, @ExpirationDate, @ParentContractNumber, @OfferId, @IsRebateRequired, @SocioBusinessSizeId, @userName, getdate(), @userName, getdate() ) -- @OptionYears
		...
		/* if this is a BPA, then update the BPALookupTable */
		select @Count = 0

		if @ParentContractNumber is not null 
		BEGIN
			exec UpdateBPALookup @userName, @ParentContractNumber, @ManuallyAssignedContractNumber, @Count OUTPUT

			select @error = @@error
		
			if @error <> 0 or @Count = 0
			BEGIN
				select @errorMsg = 'Unable to insert BPA Lookup for contract number: ' + @ManuallyAssignedContractNumber
				GOTO ERROREXIT					
			END
		END
		...
		exec InsertSAMVendorInfoForContract @CurrentUser = @CurrentUser, @SecurityServerName = @SecurityServerName, @SecurityDatabaseName = @SecurityDatabaseName, @ContractNumber = @ManuallyAssignedContractNumber, @NewContractId = @NewContractId, @ParentContractNumber = @ParentContractNumber, @SAMVendorInfoId = @SAMVendorInfoId OUTPUT

		select @error = @@error
		
		if @error <> 0 
		BEGIN
			select @errorMsg = 'Unable to insert SAM vendor info for contract number: ' + @ManuallyAssignedContractNumber
			GOTO ERROREXIT					
		END
		...
		if @ScheduleNumber = 1 OR @ScheduleNumber = 8 OR @ScheduleNumber = 18 OR  
                @ScheduleNumber = 30 OR @ScheduleNumber = 31 OR @ScheduleNumber = 32 OR @ScheduleNumber = 37 OR 
                @ScheduleNumber = 39 OR @ScheduleNumber = 47 OR @ScheduleNumber = 48 OR @ScheduleNumber = 49 OR @ScheduleNumber = 52  
		BEGIN
			...
			select @SQL = N'exec @retVal_parm = [' + @DrugItemServerName + '].[' + @DrugItemDatabaseName + '].dbo.CreateDrugItemContract @CurrentUser  = @CurrentUser_parm,
				...
				@ParentContractNumber = @ParentContractNumber_parm,
 			...
 			select @SQLParms = N'@CurrentUser_parm uniqueidentifier,  
								...
								@ParentContractNumber_parm nvarchar(50),
       		...
       		Exec SP_executeSQL @SQL, @SQLParms, @CurrentUser_parm = @CurrentUser, 
												...
												@ParentContractNumber_parm = @ParentContractNumber,
		...
		-- insert the default SIN for National or SAC contracts
		if exists ( select Schedule_Number from [tlkup_Sched/Cat] where Division in ( 2, 6 )
															and Inactive = 0															
