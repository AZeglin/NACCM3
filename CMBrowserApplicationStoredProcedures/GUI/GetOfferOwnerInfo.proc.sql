IF EXISTS ( SELECT * FROM sysobjects WHERE type = 'P' AND name = 'GetOfferOwnerInfo' )
BEGIN
	DROP PROCEDURE GetOfferOwnerInfo
END
GO

CREATE PROCEDURE GetOfferOwnerInfo
(
...
BEGIN TRANSACTION
	...
	BEGIN
		select @errorMsg = 'Error retrieving offer info for CO lookup.'
		goto ERROREXIT
	END

	select 	@AssistantDirectorCOID = Asst_Director
