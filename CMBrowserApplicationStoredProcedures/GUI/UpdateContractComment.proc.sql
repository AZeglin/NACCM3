IF EXISTS ( SELECT * FROM sysobjects WHERE type = 'P' AND name = 'UpdateContractComment' )
BEGIN
	DROP PROCEDURE UpdateContractComment
END
GO

CREATE PROCEDURE UpdateContractComment
(
...
BEGIN TRANSACTION
	...
	BEGIN
		select @errorMsg = 'Error updating contract comments for contract ' + @ContractNumber
		goto ERROREXIT
	END
