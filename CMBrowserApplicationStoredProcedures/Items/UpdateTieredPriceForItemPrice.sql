IF EXISTS ( SELECT * FROM sysobjects WHERE type = 'P' AND name = 'UpdateTieredPriceForItemPrice' )
BEGIN
	DROP PROCEDURE UpdateTieredPriceForItemPrice
END
GO

CREATE PROCEDURE UpdateTieredPriceForItemPrice
(
...
BEGIN TRANSACTION
	...
	select ItemTieredPriceId, ItemPriceId, TieredPriceStartDate, TieredPriceStopDate, Price, TierSequence, TierCriteria, MinimumValue, LastModificationType, ModificationStatusId, 0, LastModifiedBy, LastModificationDate,
			'UpdateMedSurgItemTieredPrice', @currentUserLogin, getdate() 
	...
	if @error <> 0 
	BEGIN
		select @errorMsg = 'Error moving item tiered price into history during update.'
		goto ERROREXIT
	END

	update CM_ItemTieredPrice
