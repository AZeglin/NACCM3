IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'ExpiringContractsReport')
	BEGIN
		DROP  Procedure  ExpiringContractsReport
	END

GO

CREATE Procedure ExpiringContractsReport
(
...
BEGIN TRANSACTION
	...
	exec InsertUserActivity @ReportUserLoginId, 'R', 'Expiring Contract Report', '2'
	...
	create table #ExpiringContractReport
	(
	...
	select @query = 'insert into #ExpiringContractReport
	(
		ContractNumber,
		VendorName,
		AwardDate,
	...
	update #ExpiringContractReport
	set ScheduleName = s.Schedule_Name
	from [tlkup_Sched/Cat] s join #ExpiringContractReport t on s.Schedule_Number = t.ScheduleNumber
