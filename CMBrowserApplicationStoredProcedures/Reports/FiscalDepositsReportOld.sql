...
BEGIN TRANSACTION
	...
	set CheckID = k.ID,
		...
		Difference = isnull( k.CheckAmt - ( m.VAIFF + m.OGAIFF + m.SLGIFF ), 0 ),
		DateReceived = k.DateRcvd,
		CheckNum =  k.CheckNum,
		DepositNum = k.DepositNum,
		Comments = k.Comments,
		CheckSequence =	1
	from  #FiscalDepositsByQuarterReport m join tbl_Cntrcts_Checks k on k.CntrctNum = m.CntrctNum
													and k.Quarter_ID = m.Quarter_ID
	...
	select k.CntrctNum, c.Schedule_Number, c.Contractor_Name, 0, 0, 0, 0, k.Quarter_ID, 1,
		k.ID,
		isnull( k.CheckAmt, 0 ),
	...
	and k.ID not in ( select m.CheckID from #FiscalDepositsByQuarterReport m where m.CheckID is not null )
