...
BEGIN TRANSACTION
	...
	set CheckID = k.SRPActivityId,
		...
		Difference = isnull( k.PaymentAmount - m.TotalIFF, 0 ),
		DateReceived = k.CreationDate,
		CheckNum =  k.CheckNumber,
		DepositNum = k.DepositTicketNumber,
		Comments = k.Comments,
		CheckSequence =	1
	from  #FiscalSalesVsDepositsByQuarterReport m join CM_PaymentsReceived k on k.ContractNumber = m.CntrctNum
	...
	select k.ContractNumber, c.CO_ID, c.Schedule_Number, c.Contractor_Name, 0, 0, 0, 0, k.QuarterId, 1,
		k.SRPActivityId,
		isnull( k.PaymentAmount, 0 ),
		isnull( k.PaymentAmount, 0 ),  -- no expected IFF on these records
		k.CreationDate as DateRcvd,
	...
	and k.SRPActivityId not in ( select m.CheckID from #FiscalSalesVsDepositsByQuarterReport m where m.CheckID is not null )
