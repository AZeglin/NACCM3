IF EXISTS ( SELECT * FROM sysobjects WHERE type = 'P' AND name = 'SalesByVendorReport' )
BEGIN
	DROP PROCEDURE SalesByVendorReport
END
GO

CREATE Procedure SalesByVendorReport
(
...
BEGIN TRANSACTION
	...
	exec InsertUserActivity @ReportUserLoginId, 'R', 'Sales By Vendor Report', '2'
	...
	select @query = 'insert into #SalesByVendor
	(
		ContractNumber,
		VendorName,
	...
	select @query = @query + @whereDivision + @groupByString
	...
	if @error <> 0 
	BEGIN
		select @errorMsg = 'Error selecting sales for sales by vendor report.'
		goto ERROREXIT
	END
	...
	if @error <> 0 
	BEGIN
		select @errorMsg = 'Error selecting sales by vendor report results.'
		goto ERROREXIT
	END
