IF EXISTS ( SELECT * FROM sysobjects WHERE type = 'P' AND name = 'SelectSalesQuarterlyTotalsForContractReport' )
BEGIN
	DROP PROCEDURE SelectSalesQuarterlyTotalsForContractReport
END
GO

CREATE PROCEDURE SelectSalesQuarterlyTotalsForContractReport
(
...
BEGIN TRANSACTION
	...
	SELECT dbo.tbl_Cntrcts_Sales.CntrctNum, 
			dbo.tbl_Cntrcts_Sales.Quarter_ID, 
            ...
            SUM(CAST((dbo.tbl_Cntrcts_Sales.VA_Sales * dbo.tbl_IFF.VA_IFF + dbo.tbl_Cntrcts_Sales.OGA_Sales * dbo.tbl_IFF.OGA_IFF) + dbo.tbl_Cntrcts_Sales.SLG_Sales * dbo.tbl_IFF.SLG_IFF AS decimal(19, 2))) AS [Total IFF Amount], 
			dbo.tbl_IFF.VA_IFF, 
			dbo.tbl_IFF.OGA_IFF, 
			dbo.tbl_IFF.SLG_IFF, 
            dbo.tbl_Cntrcts.Schedule_Number
	...
	GROUP BY dbo.tbl_Cntrcts_Sales.CntrctNum, 
            ...
            dbo.tbl_Cntrcts_Sales.Quarter_ID, 
			dbo.tbl_Cntrcts.Schedule_Number
	ORDER BY dbo.tlkup_year_qtr.Year DESC, dbo.tlkup_year_qtr.Qtr DESC

	select @error = @@ERROR, @rowCount = @@ROWCOUNT
	if @error <> 0 
	BEGIN
		select @errorMsg = 'Error selecting quarterly sales for report.'
		goto ERROREXIT
	END
