...
BEGIN
	...
	select	convert( nvarchar(4), q.Year ) + convert( nchar(1), q.Qtr ) as YearQtr,
			...
			k.CheckAmt, 
			isnull( CheckAmt, 0 ) - (ROUND(( m.VA * i.VA_IFF ) + ( m.OGA * i.OGA_IFF ) + ( m.SLG * i.SLG_IFF ),2)) as Difference 
	...
	left outer join tbl_Cntrcts_Checks k on k.CntrctNum = @ContractNumber
