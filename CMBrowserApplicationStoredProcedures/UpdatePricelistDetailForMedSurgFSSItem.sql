IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'UpdatePricelistDetailForMedSurgFSSItem')
	BEGIN
		DROP  Procedure  UpdatePricelistDetailForMedSurgFSSItem
	END

GO

CREATE Procedure UpdatePricelistDetailForMedSurgFSSItem
(
...
BEGIN TRANSACTION
	...
	select @PriceExists = dbo.CheckForDuplicateMedSurgFSSPriceFunction( @ExistingContractNumber, @ContractorCatalogNumber, @ProductLongDescription, @FSSPrice, @PackageSizePricedOnContract, @SIN, -1, @FSSEffectiveDate, @FSSExpirationDate, @FSSLogNumber, 0, 0 )
	...
	BEGIN
      ...
      SET   [Contractor Catalog Number] = @ContractorCatalogNumber, 
		...
		if @error <> 0 or @rowCount <> 1
		BEGIN
			select @errorMsg = 'Error updating med/surg item into pricelist.'
			goto ERROREXIT
		END
	...
	BEGIN
		select @errorMsg = 'The item/price being updated matches another item already existing in the database. If implementing a future item/price, consider inserting a new price record and choosing an effective date range that does not overlap the date range of an existing item/price.'
		goto ERROREXIT
	END
goto OKEXIT
