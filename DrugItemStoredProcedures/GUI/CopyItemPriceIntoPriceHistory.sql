IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[CopyItemPriceIntoPriceHistory]') AND type in (N'P', N'PC'))
DROP PROCEDURE [CopyItemPriceIntoPriceHistory]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE Procedure [dbo].[CopyItemPriceIntoPriceHistory]
(
...
As
	Declare @error int,@errorMsg nvarchar(1000), @UserName nvarchar(120), @retVal int,
			...
			@ExcludeFromExport bit,@LastModificationType nchar(1)
