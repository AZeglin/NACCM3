IF EXISTS (SELECT * FROM sysobjects WHERE type = 'FN' AND name = 'GetFSSPriceStartDateForItemFunction')
	BEGIN
		DROP  Function GetFSSPriceStartDateForItemFunction
	END

GO

CREATE Function GetFSSPriceStartDateForItemFunction
(
@DrugItemId int,
...
BEGIN
	...
	DECLARE @priceStartDate datetime, 
		@priceDate datetime,
		@month int,
		@year int
	...
	if @IsBPA = 1
	BEGIN
		select @priceStartDate = p.PriceStartDate
