IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'NightlyBatchProcessForRestrictedWithSubItem')
	BEGIN
		DROP  Procedure  NightlyBatchProcessForRestrictedWithSubItem
	END

GO

CREATE Proc [dbo].[NightlyBatchProcessForRestrictedWithSubItem]
As
...
Declare Price_History cursor For 
	...
	And ((DrugItemSubItemId is not null or len(DrugItemSubItemId) > 0 )
--		 and DrugItemSubItemId = -1)
		)
	order by 1
	
Open Price_History
FETCH NEXT FROM Price_History
INTO @drugitemid, @subItemId	
...
BEGIN
	...
	IsBOP,IsCMOP,IsDOD,IsHHS,IsIHS,IsIHS2,IsDIHS,IsNIH,IsPHS,IsSVH,IsSVH1,IsSVH2,IsTMOP,IsUSCG,IsFHCC,
	...
	Select DrugItemPriceId,@drugItemId,@subitemid,PriceId,PriceStartDate,PriceStopDate,Price,0,0,0,
			@IsVA,@IsBOP,@IsCMOP,@IsDOD,@IsHHS,@IsIHS,@IsIHS2,@IsDIHS,@IsNIH,@IsPHS,@IsSVH,
			@IsSVH1,@IsSVH2,@IsTMOP,@IsUSCG,@IsFHCC,null,null,null,0,'V',-1,'Nightly batch process',
