IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[TestScenario3]') AND type in (N'P', N'PC'))
DROP PROCEDURE [TestScenario3]
GO
SET ANSI_NULLS ON
GO
...
CREATE Procedure [dbo].[TestScenario3]
(
@IsFSS bit,
@IsBig4 bit,
@IsRestricted bit,
...
DECLARE
	...
	-1, 'TestScenario3', getdate(), 'TestScenario3', getdate() )
	
	select @contractId = @@IDENTITY
	
	print 'Created new contract with id= ' + convert( nvarchar(20), @contractId )
	
	exec CreateDrugItemPriceSpreadsheetModification  '{54134FF3-B915-488c-AE12-EFA8EB3AEA19}', @SimulatedContractNumber, @ModificationNumber, 'TestScenario3',
	...
	(
		...
		'TestScenario3', getdate(), 'TestScenario3', getdate()
	)
	...
	/*	exec DeleteDrugItemPrice @drugItemPriceId1, @SimulatedContractNumber, @modificationStatusId, 'TestScenario3'  */
    ...
    BEGIN -- not a removal
		...
		BEGIN
			...
			print 'creating 3 original item prices'
		    	         
		    exec InsertRawPriceForTest @drugItemId, @existingStartDate1, @existingEndDate1, 1001,
						@OriginalTemporary, @IsFSS, @IsBig4, @IsRestricted, 'T', @modificationStatusId, 'TestScenario3', @drugItemPriceId = @drugItemPriceId1 OUTPUT
						

		    exec InsertRawPriceForTest @drugItemId, @existingStartDate2, @existingEndDate2, 1002,
						@OriginalTemporary, @IsFSS, @IsBig4, @IsRestricted, 'T', @modificationStatusId, 'TestScenario3', @drugItemPriceId = @drugItemPriceId2 OUTPUT


		    exec InsertRawPriceForTest @drugItemId, @existingStartDate3, @existingEndDate3, 1003,
						@OriginalTemporary, @IsFSS, @IsBig4, @IsRestricted, 'T', @modificationStatusId, 'TestScenario3', @drugItemPriceId = @drugItemPriceId3 OUTPUT
			...
			BEGIN
				...
				@newStartDate, @newEndDate, 4545, @IsTemporary, @IsFSS, @IsBig4,
					@IsRestricted, -- VA
					0,
					0,
					0,
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						Exec  @retVal = UpdateFSSDrugItemPriceFromSpreadsheet @currentUserLogin,@SimulatedContractNumber,@drugItemPriceId1, null,
										...
										1,1.99,
										@modificationStatusId,'E',0.005,'TestScenario3'
				...
				BEGIN
					...
					@newStartDate, @newEndDate, 8484, @IsTemporary, @IsFSS, @IsBig4,
						@IsRestricted, -- VA
						0,
						0,
						0,
			...
			exec GetNextDateScenario 3, @currentStartDateNumber, @currentEndDateNumber, @nextStartDateNumber = @nextStartDateNumber OUTPUT, @nextEndDateNumber = @nextEndDateNumber OUTPUT
